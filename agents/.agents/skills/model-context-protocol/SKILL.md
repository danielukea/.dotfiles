...
description: Deep reference for the Model Context Protocol (MCP) — the JSON-RPC 2.0 standard connecting AI hosts to external tools via clients/servers. Use when building, reviewing, or debugging an MCP server/client: choosing a primitive (Tool/Resource/Prompt/Sampling/Roots/Elicitation), the `initialize` handshake, transports, OAuth 2.1, versioning, or registry publishing. Trigger on "MCP server", "tools/list", "confused deputy". Language-agnostic — see `mcp-ruby-sdk` for Ruby's API.
---
