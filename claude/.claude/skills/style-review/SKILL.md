---
name: style-review
description: Composition and pattern review of a diff (current branch, named branch, or PR) against DHH Rails composition and headless-first React style guides. Dispatches rails-architect and/or react-architect in review mode, runs a principles review (ETC, Tell-Don't-Ask, SOLID, conventions, testability, least surprise), and returns severity-tagged refactor suggestions grounded in the codebase's actual patterns. Pattern fit only — not a bug-hunting reviewer. Use whenever the user says "style review", "review this branch", "refactor suggestions", "is this DHH-style", "is this headless-first", "/style-review", or wants to evaluate composition/pattern quality of a diff before pushing.
allowed-tools: Read, Grep, Glob, Bash, Agent, AskUserQuestion, Skill
---

# Style Review

Composition/pattern review of a diff. The skill orchestrates: gather the diff, route to the right architect agents (which carry the style guides), run a principles review against the diff, and present severity-tagged findings with concrete refactors.
...
`rails-architect` and `react-architect` are explicitly built to review code as well as plan it — they already carry the style guides. This skill routes to them and adds a principles-review pass on the diff itself, then presents the unified findings.
...
Use the changed-file list to decide which architect(s) to dispatch.

| File signal | Architect |
|-------------|-----------|
| `app/models/`, `app/controllers/`, `app/jobs/`, `app/mailers/`, `db/migrate/`, `*.rb` | `rails-architect` |
| `.tsx`, `.ts`, `.jsx`, frontend `components/`, `hooks/`, `contexts/` | `react-architect` |
| Both | dispatch both in parallel |
...
## Step 3: Pre-load pattern context (optional, main thread only)

Load **at most one Rails skill and one React skill**, and only if the diff contains a pattern you can't evaluate without the guide. The architects already carry their style guides — these skills are for *your* synthesis, not theirs. When in doubt, skip.
...
## Step 4: Dispatch Architect Agent(s) in Review Mode

Use `Agent`, one call per applicable architect, in parallel. Pass each architect:
...
> <list>
>
> **Your job:** evaluate the diff against your embedded style guide (DHH composition for Rails, headless-first React for frontend).
>
> Produce:
...
**Large diffs:** if the diff exceeds ~1000 changed lines or ~20 files, ask the user whether to (a) review the highest-churn file group first, (b) review by directory, or (c) proceed against the full diff with the architect summarizing rather than line-citing. Don't truncate silently.
...
- **Use the architects' style guides, not your own.** Don't invent patterns.
